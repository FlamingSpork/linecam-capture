...
enum AccelDir parseArgDir(char *arg) {
    ...
    if(strArg == "x" || strArg == "px" || strArg == "+x") { // px -> plus x
        return POS_X;
    }
    if(strArg == "mx" || strArg == "nx" || strArg == "-x") { // mx -> minus x; nx -> negative x
        return NEG_X;
    }
...
int main(int argc, char* argv[]) {
    int c;
    bool useGPS = false, makeImage = true, badImage = false, verbose = false;
    enum AccelDir dir = POS_X;
    float multiplier = 1.0f;
    while ((c=getopt(argc, argv, "nvbgm:d:")) != -1) {
        switch(c) {
            case 'n':
                makeImage = false; // to speed up debugging by only producing velocity traces
                break;
            case 'v':
                verbose = true;
                break;
            case 'b':
                badImage = true; // naïve approach, each line in the original is a column in the output
                break;
            case 'g':
                useGPS = true;
                break;
    ...
    float lastV = 0.0f; // this is an important assumption that I try to hold to
    ...
    velocityCSV<<R"lit("abs time (s)","last AT (s)","last A (m/s/s)","last V (m/s)","n (pixels)")lit"<<endl;

//    vector<int> matSize(1, width);
    vector<int> matSize = {1, width}; // normally, these are {width, height}, but I've rotated the images to hopefully make things faster
    ...
    for(long i = 0; i < lineCount; i++) {
        ...
        // n is kinda a conversion factor from meters/s to pixels (???)
        float n = abs(lastV) * multiplier;
        if(verbose) {
            float invN = abs(lastV) > 0.0f ? 1.0f / n : 0.0f;
            cout << "Abs Time: " << absTime << " s\t AT: " << lastAT << " ms\t Accel: " << lastA << " m/s/s\t Vel: "
                 << lastV << " m/s\t 1/n: " << invN << endl;
        }
        velocityCSV<<absTime<<","<<(float)lastAT/1000.0f<<","<<lastA<<","<<lastV<<","<<n<<endl;
        if(badImage) {
            // convert and glue it onto the end
            Mat column(matSize, CV_8UC1, buf);
            vconcat(outImg, column, outImg);
        } else if(makeImage) {
            Mat column(matSize, CV_8UC1, buf);
            if(intermediate.cols == 0) {
                intermediate = column;
            }else {
                vconcat(intermediate, column, intermediate);
            }
        }

        // if current absTime > nextAT (corrected units), we need to make nextAT and nextA into lastAT and lastA and grab ourselves another line
        if(absTime > (float)( nextAT - firstAT )/1000.0f) {
            ...
            if(round(n) > 0) {
                // TODO: try this scaling-based approach vs the random approach in `grindstone (copy).cpp`
                Mat intermediateScaled;
                resize(intermediate, intermediateScaled, Size(width, round(n)), 0, 0, INTER_AREA);
                // TODO: try the various interpolation algos against each other
                vconcat(outImg, intermediateScaled, outImg);
            }
            intermediate = Mat(0,0, CV_8SC1);
        }
        if(i % 1000 == 0) {
            cout<<"line count: "<<i<<"\t time: "<<absTime<<" s"<<endl;
        }
    }
    if (makeImage) {
        cout<<"Writing image...."<<endl;
        imwrite(capDir + "out.png",
                outImg); // this is biiiig, so perhaps compressing as JPG might be better
    }
    velocityCSV.close();
}